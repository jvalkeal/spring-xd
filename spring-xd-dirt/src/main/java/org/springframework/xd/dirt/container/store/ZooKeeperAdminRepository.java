...
/**
 * ZooKeeper backed repository for runtime info about Admins. This
 ...
 */
public class ZooKeeperAdminRepository implements AdminRepository, ApplicationListener<ApplicationEvent> {
	...
	private PathChildrenCache ensureCache() {
		if (cacheRef.get() == null) {
			synchronized (cacheRef) {
				if (cacheRef.get() == null) {
					...
					PathChildrenCache cache = new PathChildrenCache(client, Paths.ADMINS,
							true, ThreadUtils.newThreadFactory("AdminCache"));
		...
		PathChildrenCache cache = cacheRef.get();
		Assert.state(cache != null, "Admin cache not initialized " +
	 ...
	 */
	@Override
	public Page<Container> findAllRuntimeContainers(Pageable pageable) {
		List<Container> containers = this.findAll();
		return pagingUtility.getPagedData(pageable, containers);
	}
